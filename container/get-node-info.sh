...
ver=$(./get-version.sh)
type="CROWN_SN"
mn_status="$(crown-cli systemnode status 2>&1)"
...
sync_status="$(crown-cli mnsync status 2>&1)"
...
case "$mn_status" in
    *"Systemnode successfully started"*)
        mn_status_level="ok"
    ;;
    *)
...
BLOCK_HASH: %s
SYNCED: %s
" "$type" "$ver" "$mn_status" "$mn_status_level" "$block_count" "$block_hash" "$sync_status" > /home/crown/.crown/node.info
